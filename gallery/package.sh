...
archive_temp="$(mktemp -d /tmp/bluemap-securitycraft-gallery.XXXXXX)"
...
(
  ...
  LC_ALL=C find . -type f -printf '%P\n' | LC_ALL=C sort |
    zip -q -X -9 "$archive_temp/securitycraft-gallery.zip" -@
)
unzip -tq "$archive_temp/securitycraft-gallery.zip"
cp "$archive_temp/securitycraft-gallery.zip" "$output_path"
